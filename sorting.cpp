...
void MJsort::RandomList(int* array, int size) {
...
void MJsort::RandomList30(int* array, int size) {
	std::chrono::steady_clock::time_point now;
	now = std::chrono::high_resolution_clock::now();
	long long seed = now.time_since_epoch().count();
...
void MJsort::RandomList32(int* array, int size) {
	std::chrono::steady_clock::time_point now;
	now = std::chrono::high_resolution_clock::now();
	long long seed = now.time_since_epoch().count();
	srand(seed % 0x100000000);
...
void MJsort::CopyArray(int* array1, int* array2, int size) {
	for (int i = 0; i < size; i++) {
		array2[i] = array1[i];
	}
}
...
bool MJsort::TestValid(int* array, int* correctArray, int size) {
	for (int i = 0; i < size; i++) {
...
double MJsort::MainSetup(int* unsorted, int* correct, int size) {
...
void MJsort::Setup(std::string* name, int* unsorted, int* toSort, int size, int i, int j, bool first, std::vector<std::string>* names) {
...
void MJsort::StandardSort(int* array, int size) {
	std::sort(array, array + size);
}

void MJsort::BubbleSort(int* array, int size) {
	bool swapped;
...
double MJsort::SortTimed(int* array, int size, functionPtr* func) {
	std::chrono::steady_clock::time_point start;
	std::chrono::steady_clock::time_point end;
	start = std::chrono::high_resolution_clock::now();
...
void MJsort::Combine2ArrayA(int* array1, int* array2, int size1, int size2) {
...
void MJsort::ThreadedSortA(int* array, int size, int threads, functionPtr* func) {
	// Split up work
...
double MJsort::ThreadedSortATimed(int* array, int size, int threads, functionPtr* func) {
	std::chrono::steady_clock::time_point start;
	std::chrono::steady_clock::time_point end;
	start = std::chrono::high_resolution_clock::now();
...
void MJsort::Combine2ArrayQ(int* array1, int* array2, int size1, int size2) {

}

void MJsort::ThreadedSortQ(int* array, int size, int threads, functionPtr* func) {
	
}

double MJsort::ThreadedSortQTimed(int* array, int size, int threads, functionPtr* func) {
	std::chrono::steady_clock::time_point start;
	std::chrono::steady_clock::time_point end;
	start = std::chrono::high_resolution_clock::now();
